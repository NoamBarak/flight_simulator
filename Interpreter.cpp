...
bool Interpreter::isNumber(const std::string &st) {
    ...
    if (((st.at(0) != '-')) && (!(isdigit(st.at(0))))) {
        cout<<"first cond in is number"<<endl;
        return false;
    }
    ...
    while (i < sLength) {
        if (st.at(i) == '.') {
            count++;
            if (count > 1) {
                cout<<"second cond in is number"<<endl;
                return false;
            }
        } else if (!isdigit(st.at(i)) && st.at(i)!='0') {
            cout<<"CHECK"<<st.at(i)<<endl;
            cout<<"third cond in is number"<<endl;
            return false;
        }
        i++;
    }
